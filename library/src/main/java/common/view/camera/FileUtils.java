...
public class FileUtils {
     ...
     * 缓存文件根目录名
     */
    private static final String FILE_DIR = "lottery";
    /**
     * 上传的照片文件路径
     */
    ...
    public static Bitmap compressBitmap(byte[] datas, int w, int h) {
        if (datas != null) {
            ...
            if (opts.outWidth != 0 && opts.outHeight != 0) {
                Lg.i("info",opts.outWidth +" "+opts.outHeight);
                ...
                Lg.i("info","compressBitmap inSampleSize "+datas.length+" "+scale);
    ...
    public static byte[] compressBitmapToBytes(Bitmap bitmap, int maxSize) {
        ...
        while (longs > maxSize && options > 0) {
            Lg.i("info","compressBitmapToBytes "+longs+"  "+options);
            baos.reset();
