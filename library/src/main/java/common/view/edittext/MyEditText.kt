...
import common.view.edittext.floatinglabeledittext.FloatingLabelEditText
import java.util.*
...
class MyEditText (context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.MyEditText, 0, 0)
        try {
            val t = ta.getInt(R.styleable.MyEditText_e_type, EditType.phone.ordinal)
            val s = ta.getString(R.styleable.MyEditText_e_text)
            s?.let { text=it }
