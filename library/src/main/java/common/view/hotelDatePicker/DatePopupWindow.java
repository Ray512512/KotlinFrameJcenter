package common.view.hotelDatePicker;
...
//    private final PopupwindowHotelDateBinding mBinding;
    ...
    public DatePopupWindow(Activity context, String date){
        ...
        View v= LayoutInflater.from(activity).inflate(R.layout.popupwindow_hotel_date,null,false);
...
//        mBinding = DataBindingUtil.inflate(LayoutInflater.from(activity), R.layout.popupwindow_hotel_date,null,false);
        ...
        tvOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(startGroupPosition==-1||startchildPosition==-1){
                    Toast.makeText(activity,"请选择入住时间", Toast.LENGTH_SHORT).show();
                    return;
                }
                if(endGroupPosition==-1||endchildPosition==-1){
                    Toast.makeText(activity,"请选择离店时间", Toast.LENGTH_SHORT).show();
                    return;
                }
...
//
    ...
    public void setDateOnClickListener(DateOnClickListener mlListener){
      ...
      public void setInit(int startGroup,int startchild,int endGroup,int endchild ){
          ...
          rv.scrollToPosition(startGroup);
      }
...
//                    showToast(item.getSingleList().get(position).getDate());
        ...
        protected void convert(BaseViewHolder helper, DayInfo item) {
            ...
            if(item.getStatus()==0){
                if(item.isSelect()){
                    ...
                    (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.main_green_half));
                }else {
                    //没选中状态
                    helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
                    helper.getView(R.id.tv_status).setVisibility(View.GONE);
                ...
                ((TextView)helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.white));
                (helper.getView(R.id.ll_bg)).setBackgroundColor(activity.getResources().getColor(R.color.main_green));
